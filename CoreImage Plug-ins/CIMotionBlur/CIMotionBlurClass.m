...
- (id)initWithManager:(PSPlugins *)manager
{
	seaPlugins = manager;
	newdata = NULL;
	[NSBundle loadNibNamed:@"CIMotionBlur" owner:self];
	return self;
}
...
- (void)run
{
    ...
    [m_angleSlider setFloatValue:30];
...
- (void)execute
{
    ...
    resdata = [self motionBlur:pluginData withBitmap:newdata];
...
//	radius = sqrt(radius);
    ...
    angle = [m_angleSlider floatValue];
