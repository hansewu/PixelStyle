...
{
	seaPlugins = manager;
	[NSBundle loadNibNamed:@"CICheckerboard" owner:self];
	return self;
}
...
- (void)run
{
    ...
    float maxValue = MAX(width, height) * 0.5;
    [m_widthSlider setMaxValue:maxValue];
    [m_widthSlider setFloatValue:maxValue * 0.05];
...
- (unsigned char *)checkerboard:(PluginData *)pluginData
{
    ...
    NSColor *foreColor = [m_colorForeground color]; //[pluginData foreColor:YES]
