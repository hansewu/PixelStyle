...
- (id)initWithManager:(PSPlugins *)manager
{
	seaPlugins = manager;
	newdata = NULL;
	[NSBundle loadNibNamed:@"CIAffineTile" owner:self];
	return self;
}
...
- (void)run
{
    ...
    [m_angleSlider setMinValue:0];
    [m_angleSlider setMaxValue:360];
    [m_angleSlider setFloatValue:0];
    [m_scaleSlider setMinValue:0];
    [m_scaleSlider setMaxValue:1];
    [m_scaleSlider setFloatValue:0.5];
    
    
    [m_positionXLabel setIntValue:[m_positionXSlider floatValue]];
    [m_positionYLabel setIntValue:[m_positionYSlider floatValue]];
    [m_scaleLabel setStringValue:[NSString stringWithFormat:@"%.2f",[m_scaleSlider floatValue]]];
    [m_angleLabel setStringValue:[NSString stringWithFormat:@"%.0f", [m_angleSlider floatValue]]];
...
//		angle = PI / 2 * ((point.y - apoint.y > 0) ? 1 : -1);
    ...
    angle = [m_angleSlider floatValue] / 180.0 * PI;
    scale = [m_scaleSlider floatValue];
