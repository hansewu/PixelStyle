...
- (id)initWithManager:(PSPlugins *)manager
{
	seaPlugins = manager;
	newdata = NULL;
	[NSBundle loadNibNamed:@"CIZoomBlur" owner:self];
	return self;
}
...
- (void)run
{
    ...
    [m_radiusSlider setMaxValue:100];
    [m_radiusSlider setFloatValue:20];
...
- (void)execute
{
    ...
    resdata = [self zoomBlur:pluginData withBitmap:newdata];
...
//	amount = sqrt(amount);
    ...
    amount = [m_radiusSlider floatValue];
