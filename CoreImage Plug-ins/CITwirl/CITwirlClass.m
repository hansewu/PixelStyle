...
- (void)run
{
    ...
    [m_angleSlider setMinValue:-12.57];
    [m_angleSlider setMaxValue:12.57];
    [m_angleSlider setFloatValue:angle];
    
	[reverseCheckbox setState:0];
    ...
    [m_angleLabel setStringValue:[NSString stringWithFormat:@"%.2f",[m_angleSlider floatValue]]];
...
- (IBAction)apply:(id)sender
{
	...
	[gUserDefaults setFloat:[m_angleSlider floatValue] forKey:@"CITwirl.angle"];
}
...
- (IBAction)changeReverseState:(id)sender
{
    float angle = -[m_angleSlider floatValue];
    [m_angleSlider setFloatValue:angle];
    [self update:nil];
}
...
- (void)execute
{
    ...
    resdata = [self twirl:pluginData withBitmap:newdata];
...
//	radius = sqrt(radius);
    ...
    radius = [m_radiusSlider floatValue];
    float angle = [m_angleSlider floatValue];
