...
- (void)awakeFromNib {
    ...
    if(![self.cell isKindOfClass:[LADSliderCell class]]) {
        self.cell = [[[LADSliderCell alloc] init] autorelease];
    }
}
...
- (id)initWithKnobImage:(NSImage *)knob barImage:(NSImage*)barImage{
    self = [super init];
    if (self) {
        self.cell = [[[LADSliderCell alloc] initWithKnobImage:knob barImage:barImage] autorelease];
...
- (id)initWithKnobImage:(NSImage *)knob minimumValueImage:(NSImage *)minImage maximumValueImage:(NSImage *)maxImage {
    self = [super init];
    if (self) {
//        if (NSEdgeInsetsEqual(minImage.capInsets, NSEdgeInsetsZero)) {
//            CGFloat inset = minImage.size.width-1;
//            if (self.sliderCell.vertical) {
//                minImage = [minImage imageRotated:90];
//                minImage.capInsets = NSEdgeInsetsMake(0, 0, inset, 0);
//            } else {
//                minImage.capInsets = NSEdgeInsetsMake(0, inset, 0, 0);
//            }
//        }
//        if (NSEdgeInsetsEqual(maxImage.capInsets, NSEdgeInsetsZero)) {
//            CGFloat inset = maxImage.size.width-1;
//            if (self.sliderCell.vertical) {
//                maxImage = [maxImage imageRotated:90];
//                maxImage.capInsets = NSEdgeInsetsMake(inset, 0, 0, 0);
//            } else {
//                maxImage.capInsets = NSEdgeInsetsMake(0, 0, 0, inset);
//            }
//        }
        self.cell = [[[LADSliderCell alloc] initWithKnobImage:knob minimumValueImage:minImage maximumValueImage:maxImage] autorelease];
        return !self.cell ? nil : self;
    }
