#import "SelectionToAlphaClass.h"

#define gOurBundle [NSBundle bundleForClass:[self class]]

#define make_128(x) (x + 16 - (x % 16))

@implementation SelectionToAlphaClass
...
- (NSString *)name
{
	return [gOurBundle localizedStringForKey:@"name" value:@"Selection to Alpha Channel" table:NULL];
}
...
- (void)run
{
    ...
    if([pluginData selectionMask:&selectionRect] == NULL)
    {
        success = NO;
        return;
    }
...
//	IntRect selection;
    ...
    IntRect selectionRect;
    unsigned char *pSelection = [pluginData selectionMask:&selectionRect];
    ...
    {
        int y1 = y - selectionRect.origin.y;
        
            for(int x=0; x< width; x++)
            {
                int x1 = x - selectionRect.origin.x;
                if(y1 >= 0 && y1< selectionRect.size.height && x1>=0 && x1 < selectionRect.size.width)
                {
                    overlay[y*width*spp +x*spp + spp -1] = pSelection[y1*selectionRect.size.width + x1];
                }
                else
                    overlay[y*width*spp +x*spp + spp -1] = 0;
            }
