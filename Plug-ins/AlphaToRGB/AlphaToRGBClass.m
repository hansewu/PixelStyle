#import "AlphaToRGBClass.h"

#define gOurBundle [NSBundle bundleForClass:[self class]]

#define make_128(x) (x + 16 - (x % 16))

@implementation AlphaToRGBClass
...
- (NSString *)name
{
	return [gOurBundle localizedStringForKey:@"name" value:@"Alpha Channel to RGB channels" table:NULL];
}
...
- (void)executeColor:(PluginData *)pluginData
{
    ...
    for(int y=0; y< height; y++)
    {
            ...
            {
                unsigned char alpha = overlay[y*width*spp +x*spp + spp -1];
                if(spp==2)
                    overlay[y*width*spp +x*spp] = alpha;
                else if(spp ==4 )
                {
                    overlay[y*width*spp +x*spp] = alpha;
                    overlay[y*width*spp +x*spp +1] = alpha;
                    overlay[y*width*spp +x*spp +2] = alpha;
                }
            }
