...
-(NSString *)toolTip
{
    return NSLocalizedString(@"Elliptical Marquee Tool", nil);
}
...
- (void)mouseDownAt:(IntPoint)where withEvent:(NSEvent *)event
{
	...
	if (![super isMovingOrScaling]) {
		...
		// Get mode
		modifier = [(EllipseSelectOptions *)m_idOptions modifier];
...
- (void)mouseUpAt:(IntPoint)where withEvent:(NSEvent *)event
{
	...
	if (m_bIntermediate && ![super isMovingOrScaling]) {
		...
		[[m_idDocument selection] selectEllipse:m_sSelectionRect mode:[m_idOptions selectionMode] feather:[m_idOptions feather]];
