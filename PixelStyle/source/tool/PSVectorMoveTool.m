...
//                
//                if (!result.node.selected) {
//                    if (![(PSVectorMoveOptions *)m_idOptions groupSelect]) {
...
//    }
//    
//    [super fineMouseUpAt:where withEvent:event];
//}
...
- (void)fineMouseDownAt:(NSPoint)where withEvent:(NSEvent *)event
{
    ...
    m_cgPointInitial.x = where.x;
    m_cgPointInitial.y = where.y;
    ...
    if (result.type == kWDEther)
    {
        ...
        if ([(PSVectorMoveOptions *)m_idOptions shiftKeyEnable])
        {
    ...
    BOOL bGroupSelect = [(PSVectorMoveOptions *)m_idOptions groupSelect];
    WDElement *element = result.element;
    ...
    if (![wdDrawingController isSelected:element]) {
        ...
        if ([element isKindOfClass:[WDPath class]] && result.node) {
            ...
            if (!nodeWasSelected_) {
                ...
                if (bGroupSelect) {
                    // only allow one node to be selected at a time
                    [wdDrawingController deselectNode:result.node];
                }
            }
...
//            [wdDrawingController deselectNode:activeNode_];
        ...
        if (m_bDragged) {
            ...
            [self moveSelectionFromPointToPointInCanvas:m_cgPointInitial toPoint:m_cgPointCurrent];
            m_cgPointInitial = m_cgPointCurrent = CGPointZero;
        } else if ([(PSVectorMoveOptions *)m_idOptions groupSelect] && lastTappedObject_ && objectWasSelected_) {
...
- (BOOL)deleteKeyPressed
{
    ...
    if ([objects count] > 0) {
        if ([objects count] == 1) {
            int selectedNodeCount = [[[objects objectAtIndex:0] selectedNodes] count];
            if (selectedNodeCount == 0) {
                [wdDrawingController deleteSelectedPath:nil];
                lastMouseMoveOnObject_ = nil;
            }else{
                [[objects objectAtIndex:0] deleteAnchors];
            }
        }else{
            [wdDrawingController deleteSelectedPath:nil];
            lastMouseMoveOnObject_ = nil;
