...
//
#include <stdio.h>
#import "PSVectorNodeEditorTool.h"
#import "PSDocument.h"
#import "PSContent.h"
#import "PSHelpers.h"
#import "PSTools.h"
...
@implementation PSVectorNodeEditorTool


- (int)toolId
{
    return kVectorNodeEditorTool;
}

-(NSString *)toolTip
{
    return NSLocalizedString(@"Node Editor Tool", nil);
}
...
- (id)init
{
    ...
    m_strToolTips = [[NSString alloc] initWithFormat:@"%@",NSLocalizedString(@"Double Click to convert the selected point. Press Command to select multiple nodes. Press Opt to operate control handles independently.", nil)];
...
//    m_transform = CGAffineTransformIdentity;
    ...
    int action = [(PSVectorNodeEditorOptions *)m_idOptions getActionStyle];
    if (action == 1) {
    ...
    BOOL bGroupSelect = [(PSVectorNodeEditorOptions *)m_idOptions groupSelect];
...
- (void)mouseDraggedTo:(IntPoint)where withEvent:(NSEvent *)event
{
    ...
    if (transformingNodes_) {
        ...
        WDBezierNodeReflectionMode reflect = [(PSVectorNodeEditorOptions *)m_idOptions optionKeyEnable] ? WDIndependent : originalReflectionMode_;
...
- (void)mouseUpAt:(IntPoint)where withEvent:(NSEvent *)event
{
    ...
    if(!m_bDragged)
    {
        ...
        WDElement *element = result.element;
        if ([wdDrawingController singleSelection]) {
            if ([element isKindOfClass:[WDPath class]] && result.node) {
                nodeWasSelected_ = result.node.selected;
                activeNode_ = result.node;
                
                if (!nodeWasSelected_) {
                    if (![(PSVectorNodeEditorOptions *)m_idOptions groupSelect]) {
                        // only allow one node to be selected at a time
                        [wdDrawingController deselectAllNodes];
                    }
                    [wdDrawingController selectNode:result.node];
    ...
    if (transformingNodes_) {
        ...
        if (m_bDragged) {
            ...
            [self moveSelectionFromPointToPointInCanvas:m_cgPointInitial toPoint:m_cgPointCurrent];
            m_cgPointInitial = m_cgPointCurrent = CGPointZero;
        } else if ([(PSVectorNodeEditorOptions *)m_idOptions groupSelect] && lastTappedObject_ && objectWasSelected_) {
