	...
	while (!finished && !ferror(file)) {
		...
		switch (propType) {
			...
			case PROP_OPACITY:
				...
				fread(tempIntString, sizeof(int), 1, file);
				fix_endian_read(tempIntString, 1);
				m_nOpacity = tempIntString[0];
				
				// Bounds checking in case of corruption
				if (m_nOpacity < 0 || m_nOpacity > 255) m_nOpacity = 255;
			break;
			case PROP_VISIBLE:
			...
			case PROP_OFFSETS:
				...
				fread(tempIntString, sizeof(int), 2, file);
				fix_endian_read(tempIntString, 2);
				m_nXoff = ((int *)tempIntString)[0];
				m_nYoff = ((int *)tempIntString)[1];
...
{
	int tilesPerRow = (m_nWidth % XCF_TILE_WIDTH) ? (m_nWidth / XCF_TILE_WIDTH + 1) : (m_nWidth / XCF_TILE_WIDTH);
	int tilesPerColumn = (m_nHeight % XCF_TILE_HEIGHT) ? (m_nHeight / XCF_TILE_HEIGHT + 1) : (m_nHeight / XCF_TILE_HEIGHT);
	...
	totalData = malloc(make_128(m_nWidth * m_nHeight * destSPP));
	...
	do {
		...
		tileWidth =  (whichTile % tilesPerRow == tilesPerRow - 1 && m_nWidth % XCF_TILE_WIDTH != 0) ? (m_nWidth % XCF_TILE_WIDTH) : XCF_TILE_WIDTH;
		tileHeight = (whichTile / tilesPerRow == tilesPerColumn - 1 && m_nHeight % XCF_TILE_HEIGHT != 0) ? (m_nHeight % XCF_TILE_HEIGHT) : XCF_TILE_HEIGHT;
		...
		if (!finished) {
				...
				case COMPRESS_NONE:
					...
					for (i = 0; i < srcSPP; i++) {
						for (j = 0; j < expectedSize; j++)
							tileData[i + j * srcSPP] = srcData[i * m_nWidth * m_nHeight + j];
					}
					free(srcData);
			...
			for (j = 0; j < tileHeight; j++) {
				for (i = 0; i < tileWidth; i++) {
					...
					destLoc = (((whichTile % tilesPerRow) * XCF_TILE_WIDTH) + i) * destSPP + ((whichTile /  tilesPerRow) * XCF_TILE_HEIGHT + j) * m_nWidth * destSPP;
...
{
	int tilesPerRow = (m_nWidth % XCF_TILE_WIDTH) ? (m_nWidth / XCF_TILE_WIDTH + 1) : (m_nWidth / XCF_TILE_WIDTH);
	int tilesPerColumn = (m_nWidth % XCF_TILE_HEIGHT) ? (m_nHeight / XCF_TILE_HEIGHT + 1) : (m_nHeight / XCF_TILE_HEIGHT);
	int tileHeight, tileWidth;
	int tileOffset, oldOffset, srcLoc, destLoc, expectedSize, srcSize;
		...
		if (!finished) {
			...
			for (j = 0; j < tileHeight; j++) {
				for (i = 0; i < tileWidth; i++) {
					...
					destLoc = (((whichTile % tilesPerRow) * XCF_TILE_WIDTH) + i) * m_nSpp + ((whichTile /  tilesPerRow) * XCF_TILE_HEIGHT + j) * m_nWidth * m_nSpp;
					totalData[destLoc + (m_nSpp - 1)] = tileData[srcLoc];
				}
			}
...
- (BOOL)readBody:(FILE *)file sharedInfo:(SharedXCFInfo *)info
{
	...
	unsigned char *pData = [self readPixels:file sharedInfo:info];
...
//    [m_pImageData reInitDataWithBuffer:pData width:m_nWidth height:m_nHeight spp:m_nSpp alphaPremultiplied:false];
	...
	if (maskOffset != 0) {
		...
		if (![self readMaskPixels:file toData:pData sharedInfo:info]) {
			return NO;
		}
	}
	else {
