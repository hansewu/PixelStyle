...
/*
- (id)initWithDocumentAfterCoder:(id)doc layer:(PSVectorLayer*)endocerLayer
{
...
- (void) createTextObjectWithTextAtPath:(NSString *)string atPoint:(CGPoint) pointStart
{
    ...
    typePath.fill = [WDColor cyanColor];//[propertyManager defaultFillStyle];
...
-(void) invalidData
{
    ...
    if(CGRectContainsRect(rectCurrent, rectText) == false)
    {
        ...
        [self drawToRawData];
        [self createCGLayer];
        [self resetDistanceAndEffectDateAndUpdate:YES];
        
    }
    else
    {
        [self drawToRawData];
        [self resetDistanceAndEffectDateAndUpdate:NO];
    }
}
...
-(void) drawToRawData
{
    ...
    [m_wdLayer renderInContext:m_contextData clipRect:CGRectMake(m_nXoff, m_nYoff, m_nWidth, m_nHeight) metaData:WDRenderingMetaDataMake(1.0, WDRenderDefault)];
...
-  (void)allocNewRawData:(int)nNewWidth height:( int )nNewHeight spp:(int)lspp opaque:(BOOL)opaque
{
    ...
    IMAGE_DATA data = [self initImageAndLockWrite:m_nWidth height:m_nHeight spp:m_nSpp];
...
-  (id)initWithDocument:(id)doc width:(int)lwidth height:(int)lheight opaque:(BOOL)opaque spp:(int)lspp  atPoint:(CGPoint)pointStart
{
    ...
    /*   // Extract appropriate values of master
     ...
     m_nXoff     = 0;//(int)pointStart.x;
     m_nYoff     = 0;//(int)pointStart.y;
     ...
     if (opaque)
     memset(m_pData, 255, m_nWidth * m_nHeight * m_nSpp);
     else
     memset(m_pData, 0, m_nWidth * m_nHeight * m_nSpp);*/
...
{
    return nil;
    //  [super initWithDocument:doc rect:lrect data:ldata spp:lspp];
    // Call the core initializer
    //  if (![self initWithDocument:doc])
    //       return NULL;
    ...
    //    m_pData = malloc(make_128(m_nWidth * m_nHeight * m_nSpp));
    //    memcpy(m_pData, ldata, m_nWidth * m_nHeight * m_nSpp);
    
    //   m_contextData =  MyCreateBitmapContext(m_nWidth , m_nHeight, m_pData, true);
...
{
    self = [super initWithDocument:doc layer:(PSLayer *)layer];
    ...
    m_pointTextStart = [layer getStartPoint];
    [self createTextObjectWithTextAtPath:@" " atPoint:m_pointTextStart];
    [self invalidData];
    ...
    /*  if (![self initWithDocument:doc])
     ...
     m_pData = malloc(make_128(m_nWidth * m_nHeight * m_nSpp));
     m_pData = memcpy(m_pData, [layer getRawData], m_nWidth * m_nHeight * m_nSpp);
     [layer unLockRawData];
     m_nXoff = [layer xoff];
     m_nYoff = [layer yoff];
...
- (id)initFloatingWithDocument:(id)doc rect:(IntRect)lrect data:(unsigned char *)ldata
{
    ...
    /*
     ...
     m_strUndoFilePath = [[NSString alloc] initWithFormat:@"/tmp/seaundo-d%d-l%d", [self uniqueLayerID], [m_idDocument uniqueDocID]];
...
- (void)decompress
{
    ...
    /*
     ...
     // Write the image data to disk
     fread(m_pData, sizeof(char), m_nWidth * m_nHeight * m_nSpp, file);
...
- (void)flipHorizontally
{
    ...
    /*
     ...
     memcpy(temp, &(m_pData[(j * m_nWidth + i) * m_nSpp]), m_nSpp);
     memcpy(&(m_pData[(j * m_nWidth + i) * m_nSpp]), &(m_pData[(j * m_nWidth + (m_nWidth - i - 1)) * m_nSpp]), m_nSpp);
     memcpy(&(m_pData[(j * m_nWidth + (m_nWidth - i - 1)) * m_nSpp]), temp, m_nSpp);
     }
     }
...
- (void)flipVertically
{
    ...
    /*
     ...
     memcpy(temp, &(m_pData[(j * m_nWidth + i) * m_nSpp]), m_nSpp);
     memcpy(&(m_pData[(j * m_nWidth + i) * m_nSpp]), &(m_pData[((m_nHeight - j - 1) * m_nWidth + i) * m_nSpp]), m_nSpp);
     memcpy(&(m_pData[((m_nHeight - j - 1) * m_nWidth + i) * m_nSpp]), temp, m_nSpp);
     }
     }
...
- (void)rotateLeft
{
    /*	int newWidth, newHeight, ox, oy;
     ...
     for (k = 0; k < m_nSpp; k++)
     {
     newData[((newHeight - i - 1) * newWidth + j) * m_nSpp + k] = m_pData[(j * m_nWidth + i) * m_nSpp + k];
     }
     }
     }
     free(m_pData);
     */
    int ox = [(PSContent *)[m_idDocument contents] width] - m_nXoff - m_nWidth;
    int oy = m_nYoff;
    
    m_Transformed = CGAffineTransformTranslate(m_Transformed, m_nWidth/2.0, m_nHeight/2.0);
...
- (void)rotateRight
{
    /*	int newWidth, newHeight, ox, oy;
     ...
     for (k = 0; k < m_nSpp; k++)
     {
     newData[(i * newWidth + (newWidth - j - 1)) * m_nSpp + k] = m_pData[(j * m_nWidth + i) * m_nSpp + k];
     }
     }
     }
     free(m_pData);
...
-(void)textChanged:(NSString *)text
{
    ...
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] textChanged:textOld];
    [textOld release];
}
...
- (NSUInteger)characterIndexForPoint:(NSPoint)aPoint
{
    ...
    for(int i=0; i< textPath.text.length; i++)
    {
        ...
        CGFloat fDist = minDistanceToRect(point, rect);
        if(fDistance > fDist)
        {
            fDistance = fDist;
            nIndex = i;
...
- (CGPathRef) getBlinkCursor
{
    ...
    if(textPath)
    {
        return  [textPath getBlinkCursor: m_nTextCursorPos];
    }
    return nil;
}
...
- (void)setFontName:(NSString *)strFontName
{
    ...
    [self invalidData];
    [(PSHelpers *)[m_idDocument helpers] layerContentsChanged:kActiveLayer];
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setFontName:fontOld];
    [fontOld release];
}
...
- (void)setFontSize:(CGFloat)fSize
{
    ...
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setFontSize:fOldSize];
}
- (void)setCustomTransform:(struct CUSTOM_TRANSFORM_)transform
{
    ...
    [self invalidData];
    [(PSHelpers *)[m_idDocument helpers] layerContentsChanged:kActiveLayer];
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setCustomTransform:oldTrans];
...
- (void)setStrokeStyle:(BOOL)bStroke width:(int)nWidth color:(NSColor *)color
{
    ...
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setStrokeStyle:YES width:nOldWidth color:colorOld];
...
- (void)setStartPoint:(CGPoint)pointStart
{
    ...
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setStartPoint:poinStartOld];
}
...
- (void)setFillColor:(NSColor *)color
{
    ...
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setFillColor:nsColorOld];
}
- (void)setFontBold:(int)nWidth
{
    ...
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setFontBold:nOldSize];
}
...
- (void)setFontItalics:(int)nItalicsValue
{
    ...
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setFontItalics:nOldSize];
}
...
- (void)setFontUnderline:(int)nUnderlineValue
{
    ...
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setFontUnderline:nOldSize];
}
...
- (void)setFontStrikethrough:(int)nStrikethroughValue
{
    ...
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setFontStrikethrough:nOldSize];
}
...
- (void)setCharacterSpace:(int)CharacterSpace
{
    ...
    [(PSVectorLayer *)[[m_idDocument undoManager] prepareWithInvocationTarget:self] setCharacterSpace:nOldSize];
}
