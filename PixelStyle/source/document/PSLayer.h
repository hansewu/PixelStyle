...
enum
{
    PSLayerStrokeEffect,
    PSLayerFillEffect,
    PSLayerOuterGlowEffect
};

@interface PSLayer : PSAbstractLayer
{
...
*/
    ...
    NSArray *m_arrOldNames;
    
    // The object responsible for changes to our bitmap
    id m_idPSLayerUndo;

    // A path to the file we use for undoing
    NSString *m_strUndoFilePath;

    // A reference to the image data representing this layer
   // unsigned char *m_pData;
    
    PSSecureImageData *m_pImageData;
    
    // Remembers whether or not the layer has an alpha channel
    BOOL m_bHasAlpha;
    
    unsigned char *m_pThumbData;
    unsigned char *m_pThumbDataRed;
    unsigned char *m_pThumbDataGreen;
    unsigned char *m_pThumbDataBlue;
    unsigned char *m_pThumbDataAlpha;
  ...
  //  BOOL m_enableEffectThreadToData;
    
    // A reference to the CGLayer representing this layer
  //  CGLayerRef m_cgLayer;
    
    //preview
  //  unsigned char *m_pPreviewData;
  //  int m_previewEffectState;
 //   NSRect m_previewRect;
  //  BOOL m_previewCancled;
  
}

// CREATION METHODS
/*!
	@method		initWithDocumentAfterCoder:layer:
 ...
 */
- (id)initWithDocumentAfterCoder:(id)doc layer:(PSLayer*)layer;
...
/*!
	@method		spp
	@discussion	Returns the samples per pixel in this layer
	@result		Returns an integer indicating the method by whcih the samples per pixel.
 */
- (int)spp;
...
- (void)setLayerEffectEnable:(BOOL)enable;
- (void)refreshLayerEffect:(BOOL)needDistance;


- (BOOL)addFilterBasedTypeID:(NSUInteger)typeID;
- (BOOL)deleteFilterAtIndex:(NSUInteger)index;
- (BOOL)moveFilterFromIndex:(NSUInteger)fromIndex toIndex:(NSUInteger)toIndex;

//update m_pDataWithFilter after apply overlay
...
//render
- (void)renderPrepare;
- (void)refreshRenderToSlice;
- (void)renderDirtyToSlice:(CGRect)rectDirty;
- (void)render:(CGContextRef) context;
/*!
	@method		renderCGLayer:inThread:
	@discussion	Updates a specified rectangle of the layer.
	@param		rect
 The rectangle to be updated.
	@param		thread
 YES if drawing should be done in thread, NO otherwise.
 */
- (void)renderCGLayer:(IntRect)rect inThread:(BOOL)thread;
