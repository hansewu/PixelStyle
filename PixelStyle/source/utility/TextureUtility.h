...
@interface TextureUtility : AbstractPanelUtility<NSTextFieldDelegate> {
	
	// The proxy object
	IBOutlet id m_idPSProxy;
	
	// The texture grouping pop-up
    IBOutlet id m_idTextureGroupPopUp;
	
	// The label that presents the user with the texture name
	IBOutlet id m_idTextureNameLabel;
...
/*!
	...
	@discussion	Frees (if necessary) and then reloads all the textures from
				PixelStyle's textures directory.
	@param		update
...
/*!
	...
	@result		Returns an instance of PSTexture representing the currently
				active texture.
*/
- (id)activeTexture;
