...
NSString *StringFromPixels(float pixels, int units, int resolution)
{
	NSString *result;
	
	switch (units) {
		case kInchUnits:
			result = [NSString stringWithFormat:@"%.2f", pixels / resolution];
		break;
		case kMillimeterUnits:
			result = [NSString stringWithFormat:@"%.0f", pixels / resolution * 25.4];
		break;
		default:
			result = [NSString stringWithFormat:@"%d", (int)roundf(pixels)];
		break;
	}
	return result;
}
