...
-(id)initWithCoder:(NSCoder *)aDecoder {
    if (self = [super initWithCoder:aDecoder]) {
        barImage = [NSImage imageNamed:@"tabviewbg"];
        ...
        for (int i=0; i < self.numberOfTabViewItems; i++) {
            //[segmentedControl setImage:[[self tabViewItemAtIndex:i] image] forSegment:i];
            [segmentedControl setLabel:NSLocalizedString([[self tabViewItemAtIndex:i] label], nil) forSegment:i];
//            [segmentedControl setLabel:[[self tabViewItemAtIndex:i] label] forSegment:i];
        ...
        [segmentedControl setFrame:NSMakeRect(0, 0, self.frame.size.width, barImage.size.height)];
    }
