...
@interface PSView : NSView {
	...
	id m_idCursorsManager;
	
	// The current zoom of the view
	float m_fZoom, m_fPrevZoom;
    int m_nZoomIndex;
	...
	// The scroll timer
	NSTimer* m_timScrollTimer;
	
	// The magnify timer
	NSTimer* m_timMagnifyTimer;
	...
	NSRulerView *m_rvHorizontalRuler;
	
	// The vertical ruler
	NSRulerView *m_rvVerticalRuler;
	
	// The vertical ruler marker
	NSRulerMarker *m_rmVMarker;
	
	// The horizontal ruler marker
	NSRulerMarker *m_rmHMarker;
	
	// The vertical stationary ruler marker
	NSRulerMarker *m_rmVStatMarker;
	
	// The horizontal stationary ruler marker
	NSRulerMarker *m_rmHStatMarker;
	
	// The mouse down location
	NSPoint m_poiMouseDownLoc;
	
	// The last active layer point
	IntPoint m_sLastActiveLayerPoint;

	// Was the key up last time?
	BOOL m_bKeyWasUp;

	// The time of the last ruler update
	NSDate *m_dateLastRulerUpdate;

	// Memorize the previous tool for a temporary eyedrop selection
	int m_nEyedropToolMemory;
	
	// Values to tell when to trigger scroll
	float m_fScrollZoom, m_fLastTrigger;

	// The amount we've magnified it the time
	float m_fMagnifyFactor;
    
    NSTimer * m_timerLoopDrawSelectBoundaries;
    
    //The points of select bound
//    POINTSET m_sSelectBoundPoints;
    NSMutableArray *m_maSelectBoundPoints;
    
    //Is update bound points
    bool m_bUpdateBoundPoints;
    
    //Is update whiteboard
    bool m_bRefreshWhiteboardImage;
    
    //The layer which is used to save the whiteboard image
    CGLayerRef m_cgLayerWhiteboardImage;
    
    id m_synthesizeImageRender;
    BOOL m_needResetCombine;
    
    CGLayerRef m_cgLayerCache;
...
///*!
//	@method		zoomIndex
//	@discussion	Returns the current zoom index of the view.
//	@result		Returns a int number representing the current zoom
// index.
// */
//- (int)zoomIndex;
...
/*!
 @method		setRefreshWhiteboardImage
 @discussion	Refresh the whiteboard image.
 @param		bRefreshWhiteboardImage
 YES if wants to refresh the layer which is used to save whiteboard image, NO otherwise..
 */
-(void)setRefreshWhiteboardImage:(bool)bRefreshWhiteboardImage;

/*!
 @method		setNeedsUpdateSelectBoundPoints
 @discussion	Update the points of selection Boundaries.
 */
...
/*!
	...
	@discussion	Ends line drawing (called by some methods in PSHelpers).
*/
- (void)endLineDrawing;
- (BOOL)isLineDrawing;
...
/*!
	@method		shutdown
	@discussion	Shut down the view.
 */
- (void)shutdown;
/*!
	@method		stopDrawSelectionBoundariesTimer
	@discussion	Stop the timer of drawing the selection Boundaries.
 */
- (void)stopDrawSelectionBoundariesTimer;

/*!
	@method		restoreDrawSelectionBoundariesTimer
	@discussion	Restore the timer of drawing the selection Boundaries.
 */
