...
- (void)setDocumentView:(NSView *)aView
{
	...
	m_bHasHorizontalScrollbar = shouldHaveHorizontalScrollbar;
	m_bHasVerticalScrollbar = shouldHaveVerticalScrollbar;
	[(NSScrollView *)[self superview] setHasHorizontalScroller:m_bHasHorizontalScrollbar];
	[(NSScrollView *)[self superview] setHasVerticalScroller:m_bHasVerticalScrollbar];
}
...
- (void)updateScrollbars:(NSRect)frameRect
{
	...
	if (!preventRecurse) {
		...
		fwo = fw + 15 * m_bHasVerticalScrollbar;
		fho = fh + 15 * m_bHasHorizontalScrollbar;
		...
		if (shouldHaveHorizontalScrollbar && !shouldHaveVerticalScrollbar) {
			scrollPoint.y = roundf((dh - fh + 15 * !m_bHasHorizontalScrollbar) / 2.0);
		} else if (!shouldHaveHorizontalScrollbar && shouldHaveVerticalScrollbar) {
			scrollPoint.x = roundf((dw - fw + 15 * !m_bHasVerticalScrollbar) / 2.0);
		} else if (!shouldHaveHorizontalScrollbar && !shouldHaveVerticalScrollbar) {
		...
		// Show/hide scrollbars
		if (shouldHaveHorizontalScrollbar != m_bHasHorizontalScrollbar) {
			m_bHasHorizontalScrollbar = !m_bHasHorizontalScrollbar;
			preventRecurse = YES;
			[(NSScrollView *)[self superview] setHasHorizontalScroller:m_bHasHorizontalScrollbar];
			preventRecurse = NO;
		}
		
		if (shouldHaveVerticalScrollbar != m_bHasVerticalScrollbar) {
			m_bHasVerticalScrollbar = !m_bHasVerticalScrollbar;
			preventRecurse = YES;
			[(NSScrollView *)[self superview] setHasVerticalScroller:m_bHasVerticalScrollbar];
			preventRecurse = NO;
		}
...
{
	if(m_evtMostRecentScrollEvent==theEvent)
	{
		[super scrollWheel:theEvent];
		return;
	}
	m_evtMostRecentScrollEvent = theEvent;
