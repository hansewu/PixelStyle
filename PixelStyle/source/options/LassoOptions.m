...
- (void)awakeFromNib
{
    ...
    //feather 初始值获取设置
    int nValue;
    if ([gUserDefaults objectForKey:@"lasso selection feather"] == NULL) {
        nValue = 0;
    }
    else {
        nValue = [gUserDefaults integerForKey:@"lasso selection feather"];
...
- (void)shutdown
{
    [gUserDefaults setInteger:[m_texFieldFeather intValue] forKey:@"lasso selection feather"];
}
